...
class DataStream(ABC):
    """base abstract class for stream processing"""
    ...
    def filter_data(self, data_batch: List[Any],
        ...
        """base method that filters data based on criterias"""
        return data_batch
...
class TransactionStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        """processes data from a batch"""
        try:
            gain = sum([int(d.split(":", 1)[1])
                        for d in data_batch if 'buy' in d])
            loss = sum([int(d.split(":", 1)[1])
                        for d in data_batch if 'sell' in d])
...
class EventStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        ...
        try:
            self.error_count += len([data for data in data_batch
                                    if 'error' in data])
...
class SensorStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        """processes data from the batch"""
        try:
            temps = [float(d.split(":", 1)[1]) for d in data_batch
                     if 'temp' in d]
    ...
    def get_stats(self) -> Dict[str, Union[str, int, float]]:
        ...
        return {"stream_id": self.stream_id,
                "processed items": self.processed_count,
                "type": "Environmental Data"}


class StreamProcessor():
    """processor class for every kind of stream"""

    def run_all(self, streams: List[DataStream], batches: List) -> None:
        """runs every kind of data stream with polymorphism"""
        ...
        print("Filtered results: 2 critical sensors alerts, "
              "1 large transaction")
